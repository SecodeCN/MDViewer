...
#### 🌐 编码检测改进
- **智能编码识别** - 重构编码检测算法，显著提高 UTF-8 和 GBK 的区分准确性
  - ✨ 新增 UTF-8 多字节序列完整性验证
  - 🎯 改进检测样本大小（1000 → 4000 字节）
  - 📊 引入编码质量评分对比机制
  - 🔍 修复 UTF-8 文件被误判为 GBK 的问题

**技术细节**：
- 新增 `checkUtf8Validity()` 方法验证 UTF-8 序列结构
- 使用可配置阈值（UTF8_VALIDITY_THRESHOLD、GBK_SCORE_MULTIPLIER）
- 删除已废弃的 `calculateGbkScore()` 方法
- 改进检测流程：BOM → ASCII → UTF-8序列验证 → 质量对比

**测试覆盖**：
- ✅ UTF-8 文件：正确检测
- ✅ GBK 文件：正确检测
- ✅ UTF-8 BOM 文件：正确检测
- ✅ ASCII 文件：正确默认为 UTF-8
