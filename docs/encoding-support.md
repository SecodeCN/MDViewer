...
2. **智能内容分析** - 如果没有 BOM，使用改进的检测算法
   - **UTF-8 序列验证** - 验证多字节序列的结构完整性（2/3/4字节序列）
   - **编码质量对比** - 当文件既可以是 UTF-8 也可以是 GBK 时，对比解码后的内容质量
   - **准确率提升** - 使用 4000 字节样本，大幅提高检测准确性

3. **检测流程**
   ```
   检测 BOM → 有 BOM → 使用 BOM 指定的编码
            ↓ 无 BOM
   检查是否纯 ASCII → 是 → UTF-8
            ↓ 否（包含高位字节）
   验证 UTF-8 序列完整性 → 有效性 > 50% → UTF-8
            ↓ 有效性 ≤ 50%
   对比 UTF-8 和 GBK 解码质量 → 选择质量更高的编码
   ```
...
1. 打开文件时，系统默认使用 UTF-8 编码
2. UTF-8 是国际标准编码，适用于大多数现代文件
3. 如果文件使用其他编码，可以手动切换

### 自动检测

1. 在编码下拉框中选择"自动检测"
2. 系统会自动检测编码（BOM、UTF-8、GBK 等）
3. 大多数情况下自动检测可以正确识别
